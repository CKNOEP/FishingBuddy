...
function FishLib:UpdateFishingSkill()
    ...
    if (fishing) then
		...
		--print("continent",continent,info)
...
function FishLib:FindBestLure(b, state, usedrinks, forcemax)
	...
	if ( level and level > 1 ) then
	   ...
	   --level = 420
	   		--print (level,skill, modifier, enchant)    
	   if ( skill <= level ) then
...
function FishLib:GetCurrentFishingLevel()
    ...
    --if LT.GetFishinglevel then
        Lo,current_max = LT:GetFishingLevel(zone)
    --print("LT:FL",current_max)
	--end
...
function FishLib:GetFishingSkillLine(join, withzone, isfishing)
    ...
    local subzone = GetSubZoneText();
    local zone = GetRealZoneText() or "Unknown";
    local Lo, Hi = LT:GetFishingLevel(zone)
	local level = self:GetCurrentFishingLevel();
    if ( withzone ) then
        part1 = zone.." : "..subzone.. " ";
    end
    ...
    elseif ( Hi ) then
         if ( Hi > 0 ) then
            local perc = totskill/Hi; -- no get aways
            if (perc > 1.0) then
                perc = 1.0;
            end
            part1 = part1.."|cff"..self:GetThresholdHexColor(perc*perc)..Hi.." ("..math.floor(perc*perc*100).."%)|r";
        else
